...
\subsection{Data Acquisition} \label{sec:data_acquisition}
    ...
    \subsection{Evaluation} \label{sec:evaluation}
        To evaluate first the early time point \glss{SS} of one scan have been plotted alongside the \gls{RPM} \gls{SS} in order to visually assess the correlation of the \glss{SS}, additionally the \gls{CC} of the whole of each \gls{SS} of all scans has also been calculated. Secondly, the \glss{SS} of one scan have been used to displacement gate that scan into $10$ bins, reconstruct, \& then execute a \gls{MC} by registering (and applying a \gls{MM} using a linear regression over the \glss{DVF} \& \gls{SS}) the volumes to the mean position. Reconstructions were performed using \gls{GE} Duetto using \gls{TOF} \gls{OSEM} with two full iterations \& $24$ subsets.%~\cite{Hudson1994}.
...
\section{Results} \label{sec:results}
    ...
    The \gls{CC} results between the \gls{RPM} \gls{SS} and all of the methods for all of the scans can be seen in~\Fref{tab:cross_correlation}
